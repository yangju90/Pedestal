package com.newegg.logistics.hydra.orm.druid.filter;

import com.alibaba.druid.filter.FilterChain;
import com.alibaba.druid.filter.stat.StatFilter;
import com.alibaba.druid.proxy.jdbc.*;
import com.newegg.logistics.hydra.core.HydraMessageSender;
import com.newegg.logistics.hydra.core.HydraSqlParameterBean;
...
public class HydraDruidFilter extends StatFilter {

    protected static final Logger logger = LoggerFactory.getLogger(HydraDruidFilter.class);


    private HydraMessageSender hydraMessageSender;


    public HydraDruidFilter(HydraMessageSender hydraMessageSender) {
        this.hydraMessageSender = hydraMessageSender;
    }
    ...
    public ConnectionProxy connection_connect(FilterChain chain, Properties info) throws SQLException {
        try{
            ...
            hydraMessageSender.sendStartMsg(instanceName,userName,"connection_connect",null);
            hydraMessageSender.sendEndMsg(false,e.getMessage(), ExceptionUtils.getStackTrace(e));
    ...
    private void sendBefore(StatementProxy statement){
        ...
        Map<Integer, JdbcParameter> parameterMap =  statement.getParameters();
        List<HydraSqlParameterBean> parameterBeans = new ArrayList<>();
        for (Integer i: parameterMap.keySet()) {
            JdbcParameter jdbcParameter = parameterMap.get(i);
            parameterBeans.add(new HydraSqlParameterBean("VARCHAR",i+"",jdbcParameter.getValue()+""));
        }
        ...
        hydraMessageSender.sendStartMsg(instanceName,userName,sql,parameterBeans);
    }
    ...
    private void sendAfter(StatementProxy statement){
        hydraMessageSender.sendEndMsg(true,null,null);
    }
