-- create database if not exists staffing default charset utf8mb4 collate utf8mb4_general_ci;
-- use staffing;



create table if not exists `ns_account_role` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '逻辑主键',

  `name` varchar(200) NOT NULL COMMENT '角色名称',
  `description` varchar(500) NULL COMMENT '角色描述',
  `role_type` varchar(20) NOT NULL COMMENT 'client 客户账号；internal  newegg人员账号；talent 人才登录账号',
  `role_status` tinyint(1) NOT NULL DEFAULT '1' COMMENT '1 启动  0 停用  默认1',

  `deleted` tinyint(1) NOT NULL DEFAULT 0,
  `in_user` varchar(200) NOT NULL COMMENT '插入用户',
  `in_date` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '插入时间',
  `last_edit_user` varchar(200) DEFAULT NULL COMMENT '最后更新用户',
  `last_edit_date` timestamp NULL DEFAULT NULL COMMENT '最后更新时间',

  PRIMARY KEY (`id`),
  INDEX `ix_ns_account_role_name` (`name`)
) ENGINE=InnoDB DEFAULT charset=utf8mb4 COMMENT='角色表';



create table if not exists `ns_account_role_menu` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '逻辑主键',
  ...
  PRIMARY KEY (`id`),
  INDEX `ix_ns_account_role_menu_role_id` (`role_id`,`menu_code`)
) ENGINE=InnoDB DEFAULT charset=utf8mb4 COMMENT='菜单权限表';



create table if not exists `ns_account_user` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '逻辑主键',

  `customer_number` varchar(200) NOT NULL COMMENT 'newegg customer中的主键ID，注册产生',
  ...
  PRIMARY KEY (`id`),
  INDEX `ix_ns_account_user_customer_number` (`customer_number`),
  INDEX `ix_ns_account_user_email` (`email`)
) ENGINE=InnoDB DEFAULT charset=utf8mb4 COMMENT='账号表';



create table if not exists `ns_system_menu` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '逻辑主键',
  ...
  PRIMARY KEY (`id`),
  INDEX `ix_ns_menu_menu_code` (`menu_code`),
  INDEX `ix_ns_menu_menu_url_prefix` (`url_prefix`)
